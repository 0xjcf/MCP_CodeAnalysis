...
describe('RedisSessionStore', () => {
  ...
  describe('TTL Management', () => {
    test('extendSessionTtl should update the expiration time', async () => {
      ...
      // Verify
      expect(mockRedisClient.expire).toHaveBeenCalledWith('mcp:session:test-session-5', 7200);
    ...
    test('getSessionTtl should return remaining TTL for a session', async () => {
      ...
      // Verify
      expect(mockRedisClient.ttl).toHaveBeenCalledWith('mcp:session:test-session-6');
